...
const getSellerActionType = (platform: string) =>
...
const Home: React.FC = () => {
  ...
  const [curatorApiKey, setCuratorApiKey] = useState<string>(() => {
    return localStorage.getItem("curatorApiKey") || "";
  });
  ...
  const canSendSellerVerify = Boolean(
    curatorApiKey.trim() && sellerVerifyTxId.trim()
  );
  const submitButtonLabel = isIdentityFlow ? "Submit Identity" : "Submit Buyer";
  const attestationResponseLabel = isSellerVerifyFlow
    ? "Seller Verify Response:"
    : isIdentityFlow
    ? "Identity Response:"
    : "Attestation Response:";
  const stepFourLabel = isSellerVerifyFlow
    ? "Seller Verify"
    : isIdentityFlow
    ? "Submit Identity"
  ...
  useEffect(() => {
    const trimmedKey = curatorApiKey.trim();
    if (trimmedKey) {
      localStorage.setItem("curatorApiKey", trimmedKey);
    } else {
      localStorage.removeItem("curatorApiKey");
    }
  }, [curatorApiKey]);
  ...
  const resolveProofRoute = (): ProofRoute => {
    ...
    const capturePlatform = isSellerFlow
      ? trimmedPaymentPlatform.toLowerCase()
      : trimmedPaymentPlatform;
    const captureActionType = isSellerFlow
      ? getSellerActionType(capturePlatform)
      : actionType.trim();
  ...
  const buildPaymentIntentDetails = (
    metadata?: GenericRecord
  ): AttestationIntentDetails => {
    const hex = resolveVerificationIntentHash();

    if (hex === ZERO_BYTES32) {
      throw new Error("Enter an intent hash before verifying the payment.");
    }
  ...
  const postAttestationJson = async (
    endpoint: string,
    payload: unknown,
  ...
  const handleAuthenticate = () => {
    if (!requiresExtensionCapture) return;
    ...
    try {
      ...
      if (isBuyerTeePaymentProofInput(proofData)) {
        ...
        const payload = {
          ...
          params: verifyParams,
          chainId,
          intent: buildPaymentIntentDetails(intentMetadata),
        };
        ...
        <AppContainer>
          ...
          <MiddlePanel>
            <Section>
              ...
              {isBuyerFlow ? (
                <>
                  {platformMetadata[metadataLookupPlatform]
                    ?.buyerTeeCapture && (
                  ...
                  {platformMetadata[metadataLookupPlatform]?.metadata ? (
                    <MetadataList>
                      {platformMetadata[metadataLookupPlatform].metadata.map(
                        (m) => {
                ...
                <EmptyStateContainer>
                  <EmptyStateMessage>
                    Seller verify inputs are in Step 4
                  </EmptyStateMessage>
                </EmptyStateContainer>
              )}
            </Section>
          ...
          {(isBuyerFlow || isSellerVerifyFlow) && (
            <VerifyPanel>
              <Section>
                <StepIndicator>
                ...
                <VerifyGrid>
                    ...
                    <AttestationControls>
                      ...
                      {isBuyerFlow && (
                        <Input
                          ...
                          onChange={(e) =>
                            setAttestationBaseUrl(e.target.value)
                          }
                          valueFontSize="14px"
                            ...
                            {isSellerVerifyAdvancedOpen && (
                              <AdvancedContent id="seller-verify-advanced-panel">
                                <Input
                                  ...
                                  value={curatorApiKey}
                                  onChange={(e) =>
                                    setCuratorApiKey(e.target.value)
                                  }
                                  valueFontSize="14px"
                                  type="password"
                                  readOnly={attestationLoading}
                        ...
                        <div style={{ display: "flex", gap: 12 }}>
                          ...
                          {isSellerVerifyFlow && (
                            <Button
                              ...
                              disabled={
                                attestationLoading || !canSendSellerVerify
                              }
                              loading={attestationLoading}
                              height={48}
                              width={216}
                  ...
                  <VerifyRight>
                    {attestationResponse && !isSellerVerifyFlow && (
                    ...
                    {isPaymentAttestationFlow && attestationResponse ? (
