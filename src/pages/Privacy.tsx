...
export const Privacy: React.FC = () => {
  ...
  return (
    <PageWrapper $isMobile={currentDeviceSize === 'tablet' || currentDeviceSize === 'mobile'}>
      ...
      <PolicySubheader>Last modified: January 24th, 2025</PolicySubheader>
      <PolicyContent>
        ...
        <PolicyContentSection>
          ● We do not collect or store personal data, such as first name, last name, street
          address, date of birth, email address, or IP address, in connection with your use of the
          Services.
        </PolicyContentSection>

        <PolicyContentSection>
          ● We enable authentication via an extension to read select payment data. This enables
          an improved user experience in generating ZK proofs. Only data explicitly approved
          by the user will be used in proof generation. At no point payment data ever leaves the user's device.
        </PolicyContentSection>
        ...
        <PolicyContentSection>
          If you specifically provide us with information (such as generated proofs and inputs), we may
          use that information for the purposes described when you provide it to us. We will not attempt to link any
        ...
        <PolicyContentSection>
          We may collect error logs or debugging information to help identify and fix issues in the proof generation process.
          For this, we use third-party error reporting services (e.g., Rollbar). We only collect limited technical details
          needed for troubleshooting (e.g., zero-knowledge proofs, smart contract simulation errors, or non-sensitive error messages).
        </PolicyContentSection>

        <PolicyContentSection>
          If you use the Services as a seller, you may choose to provide publicly identifiable payment details (e.g., your Venmo handle,
          Cash App tag, or similar identifiers) so that buyers can send you payments. These identifiers may be stored on our servers to
          improve user experience—for example, by allowing the system to provide your payment details to interested parties in a transaction.
          You may request that we remove this information at any time by contacting us at team@zkp2p.xyz.
        </PolicyContentSection>
