...
const byteArrayToHexString = (byteArray: { [key: number]: number }): string => {
  return (
    "0x" +
    Object.values(byteArray)
      .map((byte) => byte.toString(16).padStart(2, "0"))
      .join("")
  );
};

const parseExtensionProof = (proofObject: any) => {
  return {
    claimInfo: {
...
const parseAppClipProof = (proofObject: any) => {
  return {
    claimInfo: {
...
const encodeProofAsBytes = (proof: Proof) => {
 ...
 * verifies a claim.
 */
function createSignDataForClaim(data: CompleteClaimData) {
  const identifier =
 ...
 * @returns
 */
function getIdentifierFromClaimInfo(info: ClaimInfo): string {
...
const generateFakeVenmoPaymentProof = async (
  intentHash: string,
  amount: string = "1.00",
  ...
  signerWallet: Wallet = new ethers.Wallet(
  ...
  const proofObject = {
    claimInfo: {
      ...
      parameters: JSON.stringify({
        ...
        responseRedactions: [
          {
            jsonPath: "$.stories[0].amount",
            regex: '"amount":"- \\$(.*)"',
            xPath: "",
          },
          { jsonPath: "$.stories[0].date", regex: '"date":"(.*)"', xPath: "" },
          {
            jsonPath: "$.stories[0].title.receiver.id",
            regex: '"id":"(.*)"',
            xPath: "",
          },
          {
            jsonPath: "$.stories[0].paymentId",
            regex: '"paymentId":"(.*)"',
            xPath: "",
          },
        ],
        url: "https://account.venmo.com/api/stories?feedType=me&externalId={{URL_PARAMS_GRD}}",
      }),
